...
function EditChatbot({ params }: { params: { id: string } }) {
  ...
  const handleAddCharacteristics = async(content: string) => {
    try {
      ...
      console.error("Failed to add character istic", error);
    }
  }
  ...
  if(loading)
    return(
      <div className="mx-auto animate-spin p-10">
        <Avatar seed="Quanta Support Agent" className=""/>
      </div>
    )
  ...
  return (
    <div  className="px-0 md:p-10">
      <div className="md:sticky md:top-0 z-50 sm:max-w-sm ml-auto space-y-2 md:border p-5 rounded-lg bg-[#2991EE]">
        ...
        {url ? ( 
            ...
            <p className="text-white text-sm">Generating link...</p> 
          )}
      ...
      <section className="relative mt-5 bg-white p-5 md:p-10 rounded-lg">
        ...
        <div>
          <Avatar seed={chatbotName} className=""/>
          <form
        ...
        <div className="bg-gray-200 p-5 md:p-5 rounded-md mt-5">
          <form onSubmit={e => {
            ...
            <Input
            type="text"
            placeholder="Example: If customer asks for prices, provide
            pricing page: ww.example.com/pricing"
