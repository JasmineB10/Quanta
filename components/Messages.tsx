...
function Messages({messages, chatbotName}: {messages: Message[]; chatbotName: string}) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto space-y-10 py-20 px-5 bg-white rounded-lg">
        {messages
  .filter(msg => msg && msg.id !== undefined) // Remove undefined messages
  .map((message, index) => {
    const isSender = message.sender !== "user";
    const messageId = message.id ?? `fallback-${index}`; // Fallback key if id is undefined
            return (
                <div 
                   key={message.id}
                    ...
                    <div className={`chat-bubble text-white
                        ...
                        <ReactMarkdown
                           ...
                           components={{
                            ...
                            p: ({node, ...props}) => (
                                <p {...props} className={`whitespace-break-spaces mb-5
